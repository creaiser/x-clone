...
const ImageEditor = ({onClose, previewURL, settings, setSettings}:{
  ...
  return (
    <div className="fixed w-screen h-screen top-0 left-0 bg-black/75 flex items-center justify-center z-10">
      <div className="bg-black rounded-xl p-12 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <button className='py-2 px-4 rounded-full bg-white text-black font-bold cursor-pointer' onClick={onClose}>Save</button>
        </div>
        {/*Image Container*/}
        <div className="w-[600px] h-[600px] flex items-center">
          <Image src={previewURL} alt='' width={600} height={600} className={`w-full ${
        ...
        <div className="flex items-center justify-between text-sm">
            <div className="flex items-center gap-8">
              ...
              <div className="flex items-center gap-2 cursor-pointer" onClick={()=>handleChangeType("original") }>
                <svg width={24} viewBox="0 0 24 24">
                  <path className={settings.type === "original"
                    ? "fill-iconBlue"
                    : "fill-[#e7e9ea]"
                  }
              ...
              <div className="flex items-center gap-2 cursor-pointer" onClick={()=>handleChangeType("wide") }>
                <svg width={24} viewBox="0 0 24 24">
                  <path className={settings.type === "wide"
                    ? "fill-iconBlue"
                    : "fill-[#e7e9ea]"
                  }
              ...
              <div className="flex items-center gap-2 cursor-pointer" onClick={()=>handleChangeType("square") }>
                <svg width={24} viewBox="0 0 24 24">
                  <path className={settings.type === "square"
                    ? "fill-iconBlue"
                    : "fill-[#e7e9ea]"
                  }
            ...
            <div onClick={() => handleChangeSensitive(!settings.sensitive)} className={`cursor-pointer py-1 px-4 rounded-full text-black ${ settings.sensitive ? "bg-red-500" : "bg-white"}`}>
              Sensitive
            </div>
        </div>
      </div>
